...
export default function EditEventScreen({ route, navigation }) {
  ...
  async function handleUpdate() {
    if (!form.title.trim() || !form.description.trim() || !form.date || !form.location.trim()) {
      Alert.alert('Missing fields', 'Title, Description, Date, and Location are required.');
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.form}>
        ...
        <TextInput style={styles.input} value={form.location} onChangeText={(v) => updateField('location', v)} />
        ...
        <TouchableOpacity
          ...
          disabled={loading}
        >
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Save & Re-submit</Text>}
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    padding: 13, fontSize: 15, color: '#212121', backgroundColor: '#fff',
  },
  textArea: { height: 100, textAlignVertical: 'top' },
  pickerBtn: {
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#e0e0e0',
    borderRadius: 10, padding: 13, flexDirection: 'row', alignItems: 'center',
  },
  ...
  imagePicker: {
    ...
    borderRadius: 12, height: 160, justifyContent: 'center', alignItems: 'center',
    backgroundColor: '#fafafa', marginTop: 6, overflow: 'hidden',
  },
