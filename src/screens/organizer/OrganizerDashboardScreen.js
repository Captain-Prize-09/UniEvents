  ...
  return (
    <View style={styles.container}>
      <View style={styles.statsRow}>
        {[EVENT_STATUS.PENDING, EVENT_STATUS.APPROVED, EVENT_STATUS.REJECTED].map((s) => (
          <View key={s} style={[styles.statCard, { backgroundColor: STATUS_COLORS[s].bg }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.center}>
            ...
            <Text style={styles.emptyText}>No events yet.{'\n'}Tap ➕ Create to add your first event!</Text>
          </View>
        }
        renderItem={({ item }) => {
          const colors = STATUS_COLORS[item.status] ?? STATUS_COLORS[EVENT_STATUS.PENDING];
          return (
            <View style={styles.card}>
              ...
              <View style={styles.cardActions}>
                <TouchableOpacity
                  ...
                  onPress={() => navigation.navigate('EventRegistrations', { eventId: item.id, eventTitle: item.title })}
                >
                  <Text style={styles.actionIcon}>👥</Text>
                  <Text style={styles.actionLabel}>{item.registrationCount ?? 0} Registered</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={styles.actionBtn}
                  onPress={() => navigation.navigate('EventFeedback', { eventId: item.id, eventTitle: item.title })}
                >
                  <Text style={styles.actionIcon}>💬</Text>
...
const styles = StyleSheet.create({
  ...
  statLabel: { fontSize: 12, fontWeight: '600', textTransform: 'uppercase', marginTop: 2 },
  list: { padding: 14, gap: 12 },
  card: { backgroundColor: '#fff', borderRadius: 12, elevation: 2, overflow: 'hidden' },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, paddingBottom: 6 },
  cardTitle: { fontSize: 16, fontWeight: '700', color: '#212121', flex: 1, marginRight: 8 },
  badge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  badgeText: { fontSize: 12, fontWeight: '700', textTransform: 'capitalize' },
  cardMeta: { fontSize: 13, color: '#757575', paddingHorizontal: 16, paddingBottom: 10 },
  adminNote: { fontSize: 13, color: '#c62828', paddingHorizontal: 16, paddingBottom: 8, fontStyle: 'italic' },
  cardActions: {
  ...
  emptyText: { color: '#9e9e9e', fontSize: 15, textAlign: 'center', lineHeight: 22 },
});
