...
export default function EventDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    isRegistered(user.uid, event.id).then((val) => {
      setRegistered(val);
      setLoadingReg(false);
    });
  }, [user.uid, event.id]);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={styles.metaValue}>{event.date}  {event.time}</Text>
        </View>
        <View style={styles.metaRow}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={styles.metaValue}>
            {event.registrationCount ?? 0}{event.capacity ? ` / ${event.capacity}` : ''}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  body: { padding: 20 },
  categoryBadge: {
    backgroundColor: '#e8eaf6', paddingHorizontal: 12, paddingVertical: 4,
    borderRadius: 20, alignSelf: 'flex-start', marginBottom: 10,
  },
  categoryText: { color: '#1a237e', fontSize: 11, fontWeight: '700', textTransform: 'uppercase' },
  ...
  metaValue: { color: '#424242', fontSize: 14, fontWeight: '600', flex: 1, textAlign: 'right' },
  sectionHeader: { fontSize: 16, fontWeight: '700', color: '#212121', marginTop: 20, marginBottom: 8 },
  ...
  feedbackBtn: {
    borderWidth: 2, borderColor: '#1a237e', borderRadius: 12,
